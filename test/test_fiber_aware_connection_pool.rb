require 'fiber'
require File.join( __FILE__, '../helper')

class TestFiberAwareConnectionPool < Minitest::Test
  attr_reader :pool

  def setup
    # Keep a duplicate pool so we do not bother others
    @pool = FiberAwareConnectionPool.new(dbname: 'em_test')
  end
  ...
  def test_checkout_after_close_test
  ...
  def test_released_connection_moves_between_fibers
    thread_conn = nil
  ...
  def test_checkout_behaviour
    pool = FiberAwareConnectionPool.new ActiveRecord::Base.connection_pool.spec
    connection = pool.connection
    assert_not_nil connection
    fibers = []
    4.times do |i|
      fibers << Fiber.new(i) do
        connection = pool.connection
        assert_not_nil connection
        connection.close
      end
